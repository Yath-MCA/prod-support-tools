...
def _get_app_data_dir() -> Path:
    ...
    When running frozen (PyInstaller EXE), __file__ points to a read-only temp
    extraction folder, so we use Documents/IMPACT_ConfigSuite instead.
    """
    import sys
    if getattr(sys, 'frozen', False):
        # Running as EXE – use user Documents folder
        base = Path.home() / "Documents" / "IMPACT_ConfigSuite" / "analyses"
    else:
        # Running as .py script – use __file__ directory (original behaviour)
