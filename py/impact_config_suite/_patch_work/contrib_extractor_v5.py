...
For every document in documents.json (using meta.json for client / shortcode):
  ...
  4. Builds ONE report per client + shortcode -> <root>/contrib_reports/<client>_<shortcode>_contrib_v<N>.html

Version
-------
  SCRIPT_VERSION (top of the file) is bumped on every script change. It is in
  the report file names (..._contrib_v5.html, ..._elements_v5.html), shown as a
  header chip and stored in meta-contrib.json: a shortcode generated by an
  older version shows as [update] and is regenerated on the next run. Older
  versioned files stay on disk, so versions can be compared.
      ...
      v5  element list hides the default elements (DEFAULT_ELEMENTS)
...
Element inventory
-----------------
  For every shortcode the script also lists the unique element names found in
  the <contrib-group>s of its files: files containing it, total occurrences,
  how many per contrib (count: contribs, e.g. xref 0: 12 . 1: 340 . 2: 210),
  parent elements and attributes (values + counts). A second table lists the
  processing-instruction targets (pistart, attributepistart, PageID ...) with
  their distinct values, and the number of comments. Elements that are not in
  every file are marked "partial" and list the files.
  Default elements (DEFAULT_ELEMENTS: contrib, name, surname, given-names, xref)
  are not listed; a note above the table says how often they occur.
  ELEMENTS_REPORT = "same"      collapsible section inside the report
                    "separate"  <client>_<shortcode>_elements_v<N>.html, linked from the report header
                    "both"      (default)
...
-----------------------------
  ...
  not generated again:
      ...
      { "JATS": { "LWW": { "INF": {
            "generated_at": "...", "version": 5, "report": "contrib_reports/JATS/LWW_INF_contrib_v5.html",
  ...
  recorded yet, or when it was generated by an older SCRIPT_VERSION. Regenerating a shortcode always rebuilds its whole report
...
Expected layout:
    ...
    root/contrib_reports/JATS/<client>_<shortcode>_contrib_v<N>.html
...
SCRIPT_VERSION = 5               # bump on EVERY script change: goes into file names (_v5) and marks old reports as "update"
DEFAULT_ELEMENTS = {"contrib", "name", "surname", "given-names", "given-name", "xref"}  # not listed in the element list
ELEMENTS_REPORT = "both"         # element inventory: "same" (inside the report), "separate" (own file) or "both"
...
def build_inventory(rows: list[dict]) -> dict:
    """Unique element names (+ attributes, parents, per-contrib counts) and PI targets."""
    elems: dict = {}
    pis: dict = {}
    ...
    def entry(name: str) -> dict:
        return elems.setdefault(
            name,
            {"files": set(), "occ": 0, "parents": Counter(), "attrs": defaultdict(Counter), "per": Counter()},
        )
    ...
    def walk(el: ET.Element, parent: str, docid: str):
        for kid in el:
            if kid.tag is PI:
                ...
                if target == "pistart":
                    v = pi_value(kid)
                    shown = tok_val(v) if v is not None else data
                else:
                    shown = data[len(target):].strip()[:80] or "(no data)"
                e = pis.setdefault(target, {"files": set(), "occ": 0, "vals": Counter()})
                e["files"].add(docid)
                e["occ"] += 1
                e["vals"][shown] += 1
                continue
...
def visible_elements(inv: dict) -> dict:
    """Elements other than the default ones (DEFAULT_ELEMENTS)."""
    return {k: v for k, v in inv["elems"].items() if k not in DEFAULT_ELEMENTS}
...
def _files_cell(files: set, total: int, ids: dict) -> str:
    ...
    return f'<details><summary>{n}/{total} <span class="part">partial</span></summary>{shown}{more}</details>'
...
def _attrs_html(attrs: dict) -> str:
    if not attrs:
        return '<span class="none">-</span>'
    out = []
    for name, vals in attrs.items():
        if len(vals) <= 8:
            shown = ", ".join(f"{esc(v) or '(empty)'}<small>&times;{n}</small>" for v, n in vals.most_common())
        else:
...
def elements_block(inv: dict) -> str:
    ...
    shown = visible_elements(inv)
    hidden = ", ".join(
        f'<code class="tok">{esc(k)}</code><small>&times;{v["occ"]}</small>'
        for k, v in inv["elems"].items()
        if k in DEFAULT_ELEMENTS
    ) or "none"
    rows = []
    for name, e in shown.items():
        ...
        rows.append(
            ...
            f'<td>{_per_contrib(e, nc)}</td><td>{parents}</td><td>{_attrs_html(e["attrs"])}</td></tr>'
        )
    if not rows:
        rows.append('<tr><td colspan="6">no elements other than the default ones</td></tr>')
    pi_rows = []
    for target, e in inv["pis"].items():
        top = e["vals"].most_common(12)
        vals = " ".join(f'<code class="tok">{esc(v)}</code><small>&times;{n}</small>' for v, n in top)
        if len(e["vals"]) > 12:
            vals += f' <small>+{len(e["vals"]) - 12} more distinct</small>'
        pi_rows.append(
            f'<tr><td><code class="tok">&lt;?{esc(target)}?&gt;</code></td>'
            f'<td>{_files_cell(e["files"], total, ids)}</td><td>{e["occ"]}</td><td>{vals}</td></tr>'
        )
    c = inv["comments"]
    if c["occ"]:
        pi_rows.append(
            '<tr><td><code class="tok">&lt;!-- comment --&gt;</code></td>'
            f'<td>{_files_cell(c["files"], total, ids)}</td><td>{c["occ"]}</td>'
            '<td><small>not parsed as content (e.g. [AU: ...] queries)</small></td></tr>'
        )
    return (
        '<div class="elems">'
        f'<h3>Elements other than default - {len(shown)} unique names in {total} files ({nc} contribs)</h3>'
        f'<div class="legend">Default elements not listed: {hidden}</div>'
        '<table class="elem"><tr><th>Element</th><th>Files</th><th>Occurrences</th>'
        '<th>Per contrib (count: contribs)</th><th>Parent element(s)</th><th>Attributes</th></tr>'
        + "".join(rows)
        + "</table>"
        f'<h3>Processing instructions / comments - {len(inv["pis"])} PI targets</h3>'
        '<table class="elem"><tr><th>Target</th><th>Files</th><th>Occurrences</th>'
        "<th>Distinct data (top 12)</th></tr>"
        + "".join(pi_rows)
        + "</table></div>"
    )


def elements_details(inv: dict) -> str:
    return (
        '<details class="stats"><summary>Element inventory '
        f'<span class="sub">{len(visible_elements(inv))} elements other than default &middot; {len(inv["pis"])} PI targets</span>'
        f"</summary>{elements_block(inv)}</details>"
    )
...
def build_elements_report(client: str, shortcode: str, inv: dict, report_dir: Path) -> Path:
    chips = [
        ...
        ("Other elements", len(visible_elements(inv)), ""),
        ("PI targets", len(inv["pis"]), ""),
        ("Script version", f"v{SCRIPT_VERSION}", ""),
        ("Generated", now(), ""),
    ]
    out = report_dir / f"{safe_name(client)}_{safe_name(shortcode)}_elements_v{SCRIPT_VERSION}.html"
    out.write_text(
        page(f"{client} / {shortcode} - element list", chips, elements_block(inv), legend=False),
        encoding="utf-8",
    )
    return out
...
def build_report(client: str, shortcode: str, rows: list[dict], report_dir: Path, stats,
    ...
    inv_section = elements_details(inv) if inv and ELEMENTS_REPORT in ("same", "both") else ""
    body = (
        stats_html(stats)
        + inv_section
...
def process_group(
    ...
    elements_out = None
    if inv and ELEMENTS_REPORT in ("separate", "both"):
        elements_out = build_elements_report(client, shortcode, inv, report_dir)
        log(
            f"[ELEMENTS] {elements_out.relative_to(base)} "
            f"({len(visible_elements(inv))} elements other than default)"
        )
    ...
    out_path, n_off = build_report(
        client, shortcode, rows, report_dir, stats, inv,
        elements_out.name if elements_out else None,
    )
    log(f"[REPORT] {out_path.relative_to(base)} ({len(rows)} docs, {n_off} off-pattern)")

    return {
        "generated_at": now(),
        "version": SCRIPT_VERSION,
        "report": out_path.relative_to(base).as_posix(),
        "elements_report": elements_out.relative_to(base).as_posix() if elements_out else None,
        "other_elements": len(visible_elements(inv)) if inv else 0,
        "documents": len(rows),
        "failed": len(rows) - len(good),
