...
class TestStatusOf(unittest.TestCase):
    ...
    def test_update_when_version_2_under_v5(self):
        """SCRIPT_VERSION=5 treats prior version=2 (and legacy logic-only) reports as updates."""
        self.assertEqual(ce.SCRIPT_VERSION, 5)
        done = {
            "JATS": {
                "LWW": {
                    "INF": {
...
class TestXrefHelpers(unittest.TestCase):
    ...
    def test_xref_key_count_mode(self):
        self.assertEqual(ce.XREF_SUBGROUP_BY, "count")
    ...
    def test_xref_label_types_sig(self):
        self.assertEqual(ce.xref_label((3, "aff+aff+fn")), "xref 3 (aff+aff+fn)")
...
class TestBuildStatsSubgroups(unittest.TestCase):
    def test_subgroups_structure(self):
        ...
        stats = ce.build_stats(rows)
        self.assertEqual(stats["total"], 3)
        card = stats["cards"][("last", "attr")]
        self.assertIn("subgroups", card)
        self.assertIn("na", card)
        # first column n/a for 1-contrib docs; last column has two xref groups
        ...
        # pattern numbers assigned within each group
    ...
    def test_seq_diff_slot_by_slot(self):
        base = (",", " and ")
        seq = (",", ", ")
        bad, lines = ce.seq_diff(base, seq, "attr")
        self.assertEqual(bad, {1})
        self.assertEqual(len(lines), 1)
        self.assertIn("differs</b> #2", lines[0])
        self.assertIn("expected", lines[0])
        self.assertIn("found", lines[0])
...
class TestProcessGroupAndMeta(unittest.TestCase):
    ...
    def test_process_group_writes_report_and_meta(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(ce.SCRIPT_VERSION, 5)
            self.assertEqual(entry.get("version"), 5)
            report = tmp / entry["report"]
            self.assertTrue(report.exists())
            self.assertIn("_contrib_v5.html", entry["report"].replace("\\", "/"))
            html = report.read_text(encoding="utf-8")
            self.assertIn("Script version", html)
            self.assertIn("v5", html)
            self.assertTrue(entry.get("elements_report"))
            self.assertIn("_elements_v5.html", entry["elements_report"].replace("\\", "/"))
            self.assertTrue((tmp / entry["elements_report"]).exists())
            el_html = (tmp / entry["elements_report"]).read_text(encoding="utf-8")
            self.assertIn("Default elements not listed", el_html)
            self.assertIn("Other elements", el_html)
            ...
            self.assertEqual(loaded["JATS"]["LWW"]["INF"].get("version"), 5)
...
class TestDefaultElementsHidden(unittest.TestCase):
    def test_visible_elements_hides_defaults(self):
        inv = {
            "elems": {
                "contrib": {"files": {"a"}, "n": 3, "per": {}, "parents": {}, "attrs": {}},
                "name": {"files": {"a"}, "n": 3, "per": {}, "parents": {}, "attrs": {}},
                "surname": {"files": {"a"}, "n": 3, "per": {}, "parents": {}, "attrs": {}},
                "given-names": {"files": {"a"}, "n": 3, "per": {}, "parents": {}, "attrs": {}},
                "given-name": {"files": {"a"}, "n": 1, "per": {}, "parents": {}, "attrs": {}},
                "xref": {"files": {"a"}, "n": 2, "per": {}, "parents": {}, "attrs": {}},
                "contrib-group": {"files": {"a"}, "n": 1, "per": {}, "parents": {}, "attrs": {}},
                "sup": {"files": {"a"}, "n": 1, "per": {}, "parents": {}, "attrs": {}},
                "email": {"files": {"a"}, "n": 1, "per": {}, "parents": {}, "attrs": {}},
            },
            "pis": {},
            "comments": 0,
            "files": 1,
        ...
        shown = ce.visible_elements(inv)
        for name in ce.DEFAULT_ELEMENTS:
            self.assertNotIn(name, shown)
        # contrib-group and sup are NOT in DEFAULT_ELEMENTS (v5) — stay visible
        self.assertIn("contrib-group", shown)
        self.assertIn("sup", shown)
        self.assertIn("email", shown)
        self.assertEqual(set(shown), {"contrib-group", "sup", "email"})
