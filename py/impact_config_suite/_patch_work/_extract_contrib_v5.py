...
def process_group(base: Path, docs: dict, metas: dict, client: str, shortcode: str, docids: list) -> dict:
    ...
    if inv and ELEMENTS_REPORT in ("separate", "both"):
        ...
        print(f"[ELEMENTS] {elements_out.relative_to(base)} ({len(visible_elements(inv))} elements other than default)")
    ...
    out, n_off = build_report(
        client, shortcode, rows, report_dir, stats, inv, elements_out.name if elements_out else None
    )
