...
------
  ...
  * Columns: File ID | Docid | First contrib | Last before | Last contrib
    ("-" when a cell would only repeat an earlier column.)
...
--------------------------------
  ...
  its row. Files with fewer than 3 (or 2) contribs are counted as "n/a" for
  the columns they do not have.
...
-----------------------------
  ...
  not generated again:
      ...
      { "JATS": { "LWW": { "INF": {
            "generated_at": "...", "report": "contrib_reports/JATS/LWW_INF_contrib.html",
  ...
  recorded yet. Regenerating a shortcode always rebuilds its whole report
...
def role_index(n: int, role: str):
    """Index of the contrib shown in a report column (None = document has none)."""
    if role == "first":
        return 0
    if role == "before":
...
def pick_cells(contribs, raws, ctx: Ctx) -> list:
    """
    first / last-before / last as (preview_html, raw_xml) pairs,
    None where it would only repeat another column.
    """
    n = len(contribs)
...
def status_of(done: dict, client: str, shortcode: str, docids: list):
    ...
    return ("update", len(fresh)) if fresh else ("done", 0)
