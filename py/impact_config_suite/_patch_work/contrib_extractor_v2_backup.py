...
--------------------------------
  ...
  Each group is "pattern-N  <files with it>/<overall files>"; pattern-1 is the
  most common one. Every other pattern lists its files (docid) and says what
...
------------------------------------------------
  ...
  every contrib is compared; where a position has more than one distinct
  ...
  The separator after the LAST contrib of a document is not compared
  (there is normally no ", " after the last author).
...
LOGIC_VERSION = 2                # bump when column/pattern logic changes -> old reports become "update"
...
def comparable(gaps: list, is_last: bool) -> list:
    """Gaps used by the all-contrib check: no separator is expected after the last contrib."""
    if not is_last:
        return gaps
    return [g for g in gaps if not (g.key[0] == "contrib" and g.key[2] == "$")]


def expected_values(gap_lists) -> dict:
    """position -> most common value, only for positions whose values disagree."""
    counts: dict = defaultdict(Counter)
    for gaps in gap_lists:
        for g in gaps:
            counts[g.key][g.value] += 1
...
def build_ctx(gap_lists, expected: dict):
    """Return (Ctx for rendering, list of issue dicts) for one document."""
    ctx, issues = Ctx(), []
    for n, gaps in enumerate(gap_lists, 1):
        for g in gaps:
            exp = expected.get(g.key)
...
def build_stats(rows: list[dict]) -> dict:
    """Group the files of one report by pattern, for every column and both kinds."""
    total = len(rows)
    cards = {}
    for role, _ in ROLES:
        for kind, _ in KINDS:
            ...
            for r in rows:
                ...
                if i is None:
                    ...
                    continue
                groups.setdefault(r["seqs"][i][kind], []).append(r)
            ordered = sorted(groups.items(), key=lambda kv: -len(kv[1]))  # stable on ties
            patterns = []
            ...
            cards[(role, kind)] = {
                "patterns": patterns,
                "na": na,
                "base": ordered[0][0] if ordered else (),
            }
...
def render_card(kind: str, card: dict, total: int) -> str:
    ...
    for p in card["patterns"]:
        ...
        out.append(
            ...
            f'<div class="pat-h"><b>pattern-{n}</b><span class="cnt">{len(rs)}/{total}</span>'
            f'<span class="pct">{len(rs) / total * 100:.1f}%</span>'
    ...
    if na:
        out.append(
            ...
            '<span class="lbl">no contrib in that column</span></div>'
...
def stats_html(stats: dict | None) -> str:
    ...
    return (
        ...
        f'<span class="sub">pattern-N &nbsp;files with it / overall {total} files &nbsp;&middot;&nbsp; '
        ...
        "3+ contribs = First, Last before, Last</div>"
...
def cell_tags(r: dict, role: str) -> str:
    ...
    for kind, label in (("attr", "attr"), ("pos", "pos")):
        ...
        out.append(f'<span class="{cls}" title="{full} pattern">{label}: pattern-{n}</span>')
...
def read_doc(base: Path, docid: str, item: dict, meta: dict, log: LogFn = _noop_log) -> dict:
    ...
    if not raw:
        ...
        log(f"[NO CONTRIB] {docid} - {row['error']}")
        return row
    ...
    row["cmp_gaps"] = [comparable(g, i == len(contribs) - 1) for i, g in enumerate(row["gaps"])]
...
def process_group(
    ...
    expected = expected_values(gl for r in good for gl in r["cmp_gaps"])
    ...
    for r in good:
        if cancel_check():
            break
        ctx, issues = build_ctx(r["cmp_gaps"], expected)
        r["issues"] = issues
    ...
    # Only write the shortcode report when we finished every doc (or had none).
    if cancelled_mid and len(rows) < len(docids):
        log(f"[CANCELLED] {client}/{shortcode} stopped mid-shortcode "
    ...
    log(f"[REPORT] {out.relative_to(base)} ({len(rows)} docs, {n_off} off-pattern)")
...
def run_contrib_extract(
    ...
    try:
        for i, (client, shortcode) in enumerate(pairs, 1):
            ...
            if cancelled():
                # Still record if we fully processed and not all-failed
                was_cancelled = True
            ...
            if entry.get("cancelled"):
                ...
                save_done(base, done)
                if entry.get("report"):
                    last_report = str(base / entry["report"])
