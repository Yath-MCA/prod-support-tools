...
#### Folder Scan Index (faster re-scan)
When **Use folder index (faster re-scan)** is enabled (default), Folder Scan builds a durable JSON index of discovered files under:

`~/Documents/impact-support-log/indexes/<sha256(abs_source_root)>.json`

Each entry stores path, mtime, size, client, DTD, and `impact_config.xml` path (reusing `get_file_metadata` / `_config_cache`). Later scans load the index, re-stat files, refresh changed entries, and apply extension / filename / DTD / client / month filters without re-parsing every config. New files are still discovered via a walk when needed. Log lines: `Loaded folder index (N files)` / `Wrote folder index (N files)`. Index paths also feed mixed-citation `file_paths` reuse after CSS discovery.
