...
class CitationPatternExtractorTab(ttk.Frame):
    """
    Tkinter Tab for Citation Pattern Extraction.
    Scans folders for IMPACT documents, groups by type|client,
    extracts citation patterns by cite type, and generates Cite Types x Clients matrix report.
    """

    DEFAULT_REPORT_FOLDER_NAME = "impact-support-log"
    HISTORY_LIMIT = 25
    history_tool_id = "citation_pattern_extractor"
    history_tool_label = "Citation Pattern Extractor"
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header_frame,
            text="📎 CITATION PATTERN EXTRACTOR",
        ...
        tk.Label(
            header_frame,
            text="Scan IMPACT document directories, extract citation patterns (ref-type/object-type), and generate Cite Types x Clients matrix report.",
            font=("Segoe UI", 9),
            fg="#94a3b8",
            bg="#1e293b",
        ...
        self.cite_type_var = tk.StringVar(value="All")
        self.cite_type_combo = ttk.Combobox(
            settings_frame,
            textvariable=self.cite_type_var,
            values=list(ElementExtractor.CITE_TYPE_PRESETS),
            width=20,
            font=("Segoe UI", 9),
        )
        self.cite_type_combo.grid(row=3, column=1, sticky="w", pady=5)
        ...
        self.recursive_chk.grid(row=4, column=1, sticky="w", pady=5)
        ...
        # 7. Open Report After Completion
        ...
        self.run_btn = tk.Button(
            btn_frame,
            text="🚀  RUN CITATION PATTERN EXTRACTION",
    ...
    def _history_summary(self, entry: dict) -> str:
        ...
        cite_type = str(entry.get("cite_type", "")).strip() or "All"
        cite_count = entry.get("params", {}).get("cite_count", 0)
        return f"{ts} | {doc_type} | {client} | {cite_type} | {source_name} | {cite_count} cites"
    ...
    def _restore_last_history_state(self) -> None:
        ...
        cite_csv_path = str(latest.get("cite_csv_path", "")).strip()
        if cite_csv_path:
            self.last_cite_csv_path = cite_csv_path
    ...
    def _apply_history_entry(self, entry: dict) -> None:
        ...
        self.cite_type_var.set(str(entry.get("cite_type", "All")).strip() or "All")
    ...
    def _current_run_settings(self, source_path: str, output_dir: str, report_path: str,
                               csv_path: str, cite_csv_path: str,
                               total_docs: int, cite_count: int, clients: list) -> dict:
        return {
            "tool_id": self.history_tool_id,
            "tool_label": self.history_tool_label,
            "action": "citation_pattern_extraction",
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "source_path": source_path,
            "doc_type": self.type_var.get().strip(),
            "client_filter": self.client_var.get().strip(),
            "cite_type": self.cite_type_var.get().strip() or "All",
            "recursive": bool(self.recursive_var.get()),
            "output_dir": output_dir,
            "open_report": bool(self.open_report_var.get()),
            "report_path": report_path,
            "csv_path": csv_path,
            "cite_csv_path": cite_csv_path,
            "summary": (
                f"{self.type_var.get().strip()} | {self.client_var.get().strip()} | "
                f"{self.cite_type_var.get().strip() or 'All'}"
            ),
            "params": {
                "doc_type": self.type_var.get().strip(),
                "client_filter": self.client_var.get().strip(),
    ...
    def _start_extraction(self):
        ...
        self.run_btn.config(state="disabled", text="⏳ EXTRACTING CITATION PATTERNS...")
        ...
        self.scan_thread = threading.Thread(
            ...
            args=(source_path, output_dir, doc_type, client_filter, cite_type),
            daemon=True
        )
    ...
    def _run_extraction_thread(
        ...
        try:
            ...
            self._log(f"🚀 Starting Citation Pattern Extraction - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            self._log(f"  Source Path:   {source_path}")
            self._log(f"  Document Type: {doc_type}")
            self._log(f"  Client Filter: {client_filter}")
            self._log(f"  Cite Type:     {cite_type}")
            ...
            self._log(f"  Citations Found:    {result.get('cite_count', 0)}")
            ...
            history_entry = self._current_run_settings(
                ...
                self.last_csv_path,
                self.last_cite_csv_path,
                result['total_docs'],
                result.get('cite_count', 0),
                result.get('clients', [])
            )
            ...
            self.run_btn.config(state="normal", text="🚀  RUN CITATION PATTERN EXTRACTION")
            self.cancel_btn.config(state="disabled")
