...
class AnalysesTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self.style.configure("Analysis.TLabel", background="#1e293b", foreground="#94a3b8", font=("Segoe UI", 10))
        self.style.configure("AnalysisHeader.TLabel", background="#1e293b", foreground="#818cf8", font=("Segoe UI", 16, "bold"))
        self.style.configure("Analysis.TButton", font=("Segoe UI", 10, "bold"), padding=10)
        self.style.configure("AnalysisAction.TButton", background="#10b981", foreground="white")

        main_container = tk.Frame(self, bg="#1e293b", padx=30, pady=30)
        main_container.pack(fill="both", expand=True)

        # Header
        tk.Label(main_container, text="CONTENT ANALYSIS ENGINE", font=("Segoe UI", 18, "bold"), fg="#818cf8", bg="#1e293b").pack(pady=(0, 20))

        # Root Path Selection
        tk.Label(main_container, text="Root Path:", bg="#1e293b", fg="#94a3b8", font=("Segoe UI", 10)).pack(anchor="w")
        ...
        self.path_entry = tk.Entry(path_frame, textvariable=self.path_var, bg="#334155", fg="white", border=0, font=("Segoe UI", 11))
        self.path_entry.pack(side="left", fill="x", expand=True, ipady=8, padx=(0, 10))
        browse_btn = tk.Button(path_frame, text="Browse", command=self._browse_path, bg="#4f46e5", fg="white", font=("Segoe UI", 10, "bold"), border=0, padx=15)
        browse_btn.pack(side="right")

        # Parameter Input
        tk.Label(main_container, text="Document ID or Client (e.g., ACS, JCIM):", bg="#1e293b", fg="#94a3b8", font=("Segoe UI", 10)).pack(anchor="w")
        self.param_var = tk.StringVar(value=self.config.get("parameter"))
        self.param_entry = tk.Entry(main_container, textvariable=self.param_var, bg="#334155", fg="white", border=0, font=("Segoe UI", 11))
        self.param_entry.pack(fill="x", pady=(5, 10), ipady=8)

        # Recursive Checkbox
        self.recursive_var = tk.BooleanVar(value=self.config.current_config.get("recursive") if hasattr(self.config, 'current_config') else False)
        ...
        chk = tk.Checkbutton(main_container, text="Recursive Sub-directory Scan", variable=self.recursive_var, 
                            bg="#1e293b", fg="#94a3b8", activebackground="#1e293b", activeforeground="#818cf8",
                            selectcolor="#334155", font=("Segoe UI", 10))
        ...
        run_btn = tk.Button(btn_frame, text="🚀 GENERATE ANALYSIS REPORT", command=self._run_analysis, 
                           bg="#10b981", fg="white", font=("Segoe UI", 12, "bold"), border=0, pady=12)
        run_btn.pack(side="left", fill="x", expand=True, padx=(0, 10))
        
        clear_btn = tk.Button(btn_frame, text="🗑️ Clear Cache", command=self._clear_cache, 
                             bg="#ef4444", fg="white", font=("Segoe UI", 10, "bold"), border=0, padx=15, pady=10)
        clear_btn.pack(side="right")

        # Status and Info
        self.status_var = tk.StringVar(value="System Ready.")
        status_label = tk.Label(main_container, textvariable=self.status_var, bg="#1e293b", fg="#64748b", font=("Segoe UI", 10, "italic"))
        status_label.pack(pady=(20, 5))

        self.cache_info = tk.Label(main_container, text=f"Cached Analysis Items: {len(self.cache)}", bg="#1e293b", fg="#475569", font=("Segoe UI", 9))
    ...
    def _clear_cache(self):
        if messagebox.askyesno("Clear Cache", "This will remove all stored analysis results. Proceed?"):
            self.cache.clear()
    ...
    def _run_analysis(self):
        ...
        try:
            ...
            if not all_dirs:
                ...
                messagebox.showinfo("Scanner", "No document folders containing 'impact_config.xml' were found in the specified path.")
                return
            ...
            for i, doc_dir in enumerate(all_dirs):
                ...
                if not is_id_mode and parameter and parameter.lower() not in cfg["client"].lower() and parameter.lower() not in doc_dir.name.lower():
                    continue
                
                self.status_var.set(f"Processing ({i+1}/{len(all_dirs)}): {doc_dir.name}")
                self.update_idletasks()
                ...
                for cat in data["categories"].values():
                    for itm in cat["items"]:
                        if itm["label"] != "N/A": global_stats["labels"] += 1
            ...
            if not doc_results:
                ...
                messagebox.showinfo("Analyzer", "Found directories but no relevant content matching the filters.")
                return
            ...
            # Save report
            out_name = f"Analysis_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
            report_path = self.config.BASE_DIR / out_name
            
            # Save copies for traceability as in recovered code
            additional_paths = [dr["path"] / f"Analysis_Report_Local.html" for dr in doc_results]
