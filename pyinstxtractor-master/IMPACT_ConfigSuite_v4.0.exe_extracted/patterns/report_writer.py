...
def write_json(data, patterns, path):
    # Build percentage-related stats from patterns
    stats = {}
...
def write_excel(data, patterns, path):
    ...
    # Summary sheet for percentage stats built from patterns
    ...
    stats = {
        'ref_numbered': calc_category_stats(patterns.get('ref_numbered', {})),
        'ref_unnumbered': calc_category_stats(patterns.get('ref_unnumbered', {})),
        'ref_footnote': calc_category_stats(patterns.get('ref_footnote', {})),
        'figtab': calc_category_stats(patterns.get('figtab', {})),
        'partlab_figure': calc_category_stats(patterns.get('partlab_figure', {})),
        'partlab_table': calc_category_stats(patterns.get('partlab_table', {})),
    }
    ...
    for c in summary_ws[1]: c.font = Font(bold=True)
    ...
    # Per-category detailed sheets with pattern rows
    for key, title in [('ref_numbered', 'Reference_Numbered_Patterns'),
                       ('ref_unnumbered', 'Reference_Unnumbered_Patterns'),
                       ('ref_footnote', 'Reference_Footnote_Patterns'),
                       ('figtab', 'FigureTable_Patterns'),
                       ('partlab_figure', 'PartLabel_Figure_Patterns'),
                       ('partlab_table', 'PartLabel_Table_Patterns')]:
        cat = stats.get(key, {})
        pats = cat.get('patterns', [])
    ...
    wb.save(path); log(f"Excel saved {path}")
